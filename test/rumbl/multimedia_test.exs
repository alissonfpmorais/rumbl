defmodule Rumbl.MultimediaTest do
  ...
  describe "videos" do
    ...
    test "create_video/2 with valid data creates a video", %{user: owner} do
      ...
      assert video.description == Map.get(@valid_attrs, :description)
      assert video.title == Map.get(@valid_attrs, :title)
      assert video.url == Map.get(@valid_attrs, :url)
    end
    ...
    test "update_video/2 with valid data updates the video", %{user: owner} do
      ...
      assert {:ok, %Video{} = video} = Multimedia.update_video(video, %{title: updated_title})
